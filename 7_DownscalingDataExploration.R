...
##### Visualize downscaling model performance as time-series (see alternative visualization below for the code that generated visualizations included in appendix of report)

run_table <- read.csv('data/run_settings_downscaling_final.csv', row.names = 1)
...
for(i in 1:nrow(run_table)){
  ...
  for(i in 1:nrow(LUT)){
    ...
    error_summary_files <- list.files(run_filepath, pattern = glob2rx('3_*_ranger_full_*error_summary', suffix, '.csv'))
  ...
  for(file in error_summary_files){
    ...
    model <- sub('.*_ranger_full_', '', sub('_error_summary_newPathWithCV', '', sub('.csv', '', file)))
...
############ Alternative Visualization (used to generate plots from Appendix of report)
...
##### OTHER ATTEMPS AT DATA INTERPRETATION AND VISUALIZATION [DEPRECATED] ######
