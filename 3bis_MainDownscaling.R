...
for(i in 1:nrow(run_table)){
  ...
  # run function 
  createLUT(modis_filepath, landsat_filepath, ID, week, output_directory = 'data/', output_suffix = suffix)
  
  
  # load LUT
  LUT <- readRDS(paste0(run_filepath, '3_c1_MODISLandsatLUT', suffix, '.RData'))
  ...
  for(file in landsat_image_list){
    createPredictionCovariatesSet(landsat_filepath, file, ID, week, b_ratios, output_suffix = suffix)
  }
