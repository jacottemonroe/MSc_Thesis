...
# Functions that takes the elephant step dataset with its covariates and fits the step-selection function to it. 
# First function fits unconditional logistic regression as generalized linear model with k-fold cross-validation.
# Second function fits conditional logistic regression without cross-validation.
# 
# Input: input dataset repository, elephant ID and week of interest, method used for generating pseudo-absence data (for recalling files by name), 
#         downscaling setting (for naming), any input or output suffix, output directory.
# 
# Output: Model coefficients, VIF, performance metrics... Saved as csv. 
...
## Function to fit unconditional logistic regression (GLM) with k-fold cross-validation

fitGLMModel <- function(input_directory = 'data/', ID, week, random_data_method = 'random_path_custom_distr', 
  ...
  write.csv(glm_coef, paste0(output_filepath, output_number, '1_glm', model_type, 'coefs_', random_data_method, suffix, '.csv'))
  write.csv(glm_deviances, paste0(output_filepath, output_number, '2_glm', model_type, 'deviances_', random_data_method, suffix, '.csv'))
  write.csv(glm_vif, paste0(output_filepath, output_number, '3_glm', model_type, 'vif_', random_data_method, suffix, '.csv'))
  write.csv(glm_validation, paste0(output_filepath, output_number, '4_glm', model_type, 'test_results_', random_data_method, suffix, '.csv'))
...
# Function to fit conditional logistic regression (CLR) without cross-validation

fitCLRModel <- function(input_directory = 'data/', ID, week, random_data_method = 'random_path_custom_distr', 
                                   downscaling = 'NULL', input_suffix = '', output_directory = 'output/', output_suffix = ''){
...
################### OTHER MODELLING ATTEMPTS [DEPRECATED] #######################
