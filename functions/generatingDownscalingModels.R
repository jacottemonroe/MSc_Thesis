...
generateDownscalingModels <- function(modis_filename, modis_date, landsat_filename, bands_to_remove_list = NULL, 
                                        ...
                                        fit_cubist_regression = F, fit_rf_regression = F){
  ...
  source('functions/removingSpatRasterLayers.R')
  ...
  source('functions/generatingCovariatesResponseSet.R')
  ...
  saveRDS(dataset, paste0('output/models/',modis_date,'_dataset.RData'))
  ...
  if(check_multicolinearity == T){
    ...
    source('functions/checkingMulticolinearity.R')
    ...
    saveRDS(multicol_results, paste0('output/models/',modis_date,'_multicol_check.RData'))
  }
  
  
  ############### create training and validation datasets #################
  
  source('functions/spatialSplittingTrainValidationSets.R')
  ...
  if(fit_linear_regression == T){
    # train model
    source('functions/fittingLinearRegression.R')
    
    lr <- fitLinearRegression(d_train, full_model = F, stepwise_AIC = T)
    
    # validate model
    source('functions/predictingModelOnData.R')
    ...
    saveRDS(lr, paste0('output/models/',modis_date,'_lr_model.RData'))
    saveRDS(lr_outputs, paste0('output/models/',modis_date,'_lr_validation.RData'))
  }
  ...
  if(fit_cubist_regression == T){
    # train model
    source('functions/fittingCubistRegression.R')
    ...
    saveRDS(cr, paste0('output/models/',modis_date,'_cr_model.RData'))
    saveRDS(cr_outputs, paste0('output/models/',modis_date,'_cr_validation.RData'))
  }
  ...
  if(fit_rf_regression == T){
    # train model
    source('functions/fittingRFRegression.R')
    ...
    saveRDS(rf, paste0('output/models/',modis_date,'_rf_model.RData'))
    saveRDS(rf_outputs, paste0('output/models/',modis_date,'_rf_validation.RData'))
  }
}