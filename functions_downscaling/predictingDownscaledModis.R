...
predictMODIS30 <- function(modis_image, modis_date, landsat_image, model_type = 'rf', 
  ...
  modis_250 <- rast(paste0('data/modis/',modis_image))
  names(modis_250) <- c('B2', 'B1')
  
  # retrieve downscaling model for corresponding date 
  model_path <- paste0('output/models/',modis_date,'_',model_type,'_model.RData')
  ...
  l_30 <- rast(paste0('data/l8/',landsat_image))
  ...
  writeRaster(modis_30_pred, paste0('output/30m_images/', modis_date, '_ndvi_30m.tif'), overwrite = T)
