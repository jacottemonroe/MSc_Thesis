...
fitMovementModelWithoutCV <- function(input_directory = 'data/', ID, week, random_data_method = 'random_path_custom_distr', 
  ...
  glm_validation <- data.frame(test_AUC = cp$AUC, test_accuracy = cp$acc, test_F1_score = cp$F1_score, 
                               ...
                               test_specificity = cp$specificity, test_positive_class = cp$pos_class, test_prevalence = cp$prevalence)
...
# output_suffix <- '_newPathWithCV50'
# 
# 
...
# test_steps <- test_steps[complete.cases(test_steps[,c('case_', 'ndvi_mean_scaled', 'ndvi_sd_scaled',
#                                                       'ndvi_rate_mean_scaled', 'ndvi_rate_sd_scaled')]),]
# training_steps <- training_steps[complete.cases(training_steps[,c('case_', 'ndvi_mean_scaled', 'ndvi_sd_scaled',
#                                                       'ndvi_rate_mean_scaled', 'ndvi_rate_sd_scaled')]),]
# 
# 
# 
...
# # 
# # 
# # customSummaryFunction <- function (data, lev = NULL, model = NULL){
...
# #     pr_auc <- NA
# #   c(AUC = pr_auc, Precision = precision.default(data = data$pred, reference = data$obs, relevant = lev[2]), 
...
# cv_settings <- trainControl(method = 'cv', number = 10, classProbs = T, summaryFunction = prSummary)
# 
# # # fit a custom GLM model through the training data with hyperparameter tuning of threshold probability
# # # source: https://www.r-bloggers.com/2015/09/how-to-perform-a-logistic-regression-in-r/
# # # source: https://www.rdocumentation.org/packages/traineR/versions/2.2.0/topics/train.glm
...
# # package: caret
# # source: https://www.r-bloggers.com/2015/09/how-to-perform-a-logistic-regression-in-r/
# glm_model <- train(case_ ~ ndvi_mean_scaled + ndvi_sd_scaled + ndvi_rate_mean_scaled + ndvi_rate_sd_scaled, method = 'glm',
#                  family = binomial(link = 'logit'), data = training_steps, trControl = cv_settings, metric = 'AUC', na.action = na.pass)
# 
# 
# ######## STEP 3: Predict test set with trained, optimized, and validated GLM model
# 
