...
## R script that runs function to extract covariates from the movement steps and MODIS images. 
## Script takes the run settings to retrieve the step_xyt dataset and the MODIS images. 
## The images are matched to each step. The NDVI is retrieved along each step, from which, the mean and standard deviation values are calculated. 
## The change in NDVI between the image at the time of movement and one week prior is also calculated, the mean and standard deviation are derived. 
...
## Script outputs: 
##    4_a1: The movement step dataset with retrieved covariates, saved as csv.
...
###########
# load function
source('functions/4_a_extractingCovariates.R')
